...
class array_iter_base
	: public boost::iterator_facade<array_iter_base<TT, forward>
									, TT
...
private:
	explicit array_iter_base(TT * e)
		: elm(e)
	{ }

	TT & dereference() const
	{
		return * elm;
	}

	template <class U>
	bool equal(array_iter_base<U, forward> const& o) const
	{
		return elm == o.elm;
	}
	...
	{
		if (forward) elm += n; else elm -= n;
	}

	ptrdiff_t distance_to(array_iter_base const & o) const
	{
...
///////////////////////////////////////////////////////////////////////////////
//
// ASIDE about the C++ language: A type is said to be trivially constructible if
// it has no user-defined constructors and all its members are trivially
// constructible. `new T` will allocate memory for a T-element, and if T is
// trivially constructible, the memory will be left uninitialized. This goes for
// arrays (T[]) as well, meaning an array initialization of a trivially
// constructible type takes practically no time.
//
...
public:
	...
	using const_iterator = array_iter_base<T const, true>;

	/// \brief Reverse iterator over a const array
	using const_reverse_iterator = array_iter_base<T const, false>;

	/// \brief Iterator over an array
	using iterator = array_iter_base<T, true>;

	/// \brief Reverse iterator over an array
	using reverse_iterator = array_iter_base<T, false>;
	...
	bool operator==(const array & other) const
	{
		...
		for (size_t i=0;i<size();++i)
			if (*get_iter(i) != *other.get_iter(i)) return false;
		return true;
	}
	...
	bool operator!=(const array & other) const
	{
		...
		for (size_t i=0; i<size(); ++i)
			if (*get_iter(i) != *other.get_iter(i)) return true;
		return false;
	}
...
public:
	...
	{
		return static_cast<double>(sizeof(array));
	}
...
private:
	...
	///
	/// \pre m_elements == null pointer; m_size == no. of elements;
	///
	/// Effect: Allocates the m_elements buffer.
	///
	/// \param copy_from  Source elements in [copy_from, copy_from+m_size)
	///////////////////////////////////////////////////////////////////////////
	void alloc_copy(const T * copy_from)
	{
		bits::allocator_usage<T, Allocator>::alloc_copy(*this, copy_from);
	}
	...
	///////////////////////////////////////////////////////////////////////////
	void alloc_fill(const T & elm)
	{
		bits::allocator_usage<T, Allocator>::alloc_fill(*this, elm);
	}
	...
	void alloc_dfl()
	{
		bits::allocator_usage<T, Allocator>::alloc_dfl(*this);
	}

	///////////////////////////////////////////////////////////////////////////
	/// \brief Destruct and deallocate elements.
	///
	/// \pre m_elements == pointer to buffer of m_size elements;
	///
	/// Effect: m_elements == null pointer; does not modify m_size
	///////////////////////////////////////////////////////////////////////////
	void destruct_and_dealloc()
	{
		bits::allocator_usage<T, Allocator>::destruct_and_dealloc(*this);
	}

	///////////////////////////////////////////////////////////////////////////
	/// Whether we allocated m_elements as a trivial_same_size<T> See the
	/// implementation note in the source for an explanation.
	///////////////////////////////////////////////////////////////////////////
	bool m_tss_used;
